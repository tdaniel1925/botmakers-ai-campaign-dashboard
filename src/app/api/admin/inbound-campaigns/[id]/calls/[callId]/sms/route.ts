...
/**
 * GET /api/admin/inbound-campaigns/[id]/calls/[callId]/sms
 * Get SMS logs for a specific inbound call
 */
  ...
  try {
    ...
    const { data: smsLogs, error: smsError } = await supabase
      ...
      .select(`
        ...
        sent_at,
        created_at,
        rule:inbound_campaign_sms_rules(
          id,
          name,
          trigger_condition
        )
    ...
    const transformedLogs = (smsLogs || []).map((sms: any) => {
      ...
      return {
        ...
        twilioSid: sms.twilio_message_sid,
